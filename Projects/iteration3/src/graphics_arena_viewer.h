...
/**
   ...
   *
   * This calls arena_->reset() 
   */

  void OnResetBtnPressed();

/**
   * @brief Handle the user pressing the hunger button on the GUI.
   *
   *  
   */

  void OnHungerBtnPressed();
  ...
  /**
   * @brief Called by arena when the game is won or lost, freezes game and overrides in-game menu
   * with new restart menu
   * @param[in] ctx Context for nanogui.
   * @param[in] current game status
   *
   */

  void VictoryOrDefeat(NVGcontext *ctx, int status);
 ...
 private:
   ...
   */
  void DrawEntity(NVGcontext *ctx, class ArenaEntity * entity);
  ...
  int light_count_{4};
  int food_count_{4};
  int fearful_count_{5};
  int light_sensitivity_{5};

  // buttons
  nanogui::Button *playing_button_{nullptr};
  nanogui::Button *reset_button_{nullptr};
  nanogui::Button *hunger_button_{nullptr};
};
