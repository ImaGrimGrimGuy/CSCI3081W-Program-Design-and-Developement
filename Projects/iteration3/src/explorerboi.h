...
#ifndef SRC_EXPLORERBOI_H_
#define SRC_EXPLORERBOI_H_
 ...
 ******************************************************************************/
/**
 * @brief Class representing an eplorerboi within the arena.
 *
 * ExplorerBois are composed of a motion handler, and a motion behavior.
 * These classes interact to maintain the pose (position and heading) of the
 * explorerboi. At each time step, the wheel velocities are used to calculate the
 * next pose of the explorerboi. The handler manages the pose and user requests. The
 * behavior calculates the new pose based on wheel velocities.
 *
 * Explorerbois can be controlled through keypress, which modify wheel velocities.
 *
 * The touch sensor is activated when the explorerboi collides with an object.
 * The heading is modified after a collision to move the explorerboi away from the
 * other object.
 */
class ExplorerBoi : public ArenaMobileEntity {
 public:
  /**
   * @brief Constructor.
   */
  ...
  ExplorerBoi();
  ExplorerBoi(const ExplorerBoi& other) = delete;
  ExplorerBoi operator=(const ExplorerBoi& other) = delete;

  /**
   * @brief Reset the ExplorerBoi to a newly constructed state (needed for reset
  ...
  /**
   * @brief Update the ExplorerBoi's position and velocity after the specified
   * duration has passed.
   *
   ...
   * @param object_type the type of the object colliding with the entity
   * @param object the object colliding with the entity
   */
  void HandleCollision(EntityType object_type, ArenaEntity * object = NULL);

  /**
   * @brief Get the name of the ExplorerBoi for visualization and for debugging.
   */
  std::string get_name() const override { return "ExplorerBoi"; }

/**
  * @brief Getter for motion_handler_
  */
  MotionHandlerExplorer get_motion_handler() {return motion_handler_;}
...
/**
 ...
 private:
  ...
  MotionHandlerExplorer motion_handler_;
  // Calculates changes in pose based on elapsed time and wheel velocities.
  MotionBehaviorDifferential motion_behavior_;
  // sensors are kept here so they are easy to move with the explorerboi
  // Lives are decremented when the explorerboi collides with anything.
