...
class Arena {
 public:
  ...
  /**
   * @brief add a controllable robot in the arena
   */
  void AddRobot();

  /**
   * @brief add an entity to the arena with entity factory
   * @param type the type of robot to add to the arena
   * @param quantity the quantity of that entity to add
   */
  void AddEntity(EntityType type, int quantity);

  /**
   * @brief accept user input to change the simulation
   * @param com the comunication (an enum)
   */
  ...
  /**
   * @brief Overloaded reset for resetting with sliders.
   *
   * @param numLights the number of lights to put in the arena
   * @param numBots the number of bots to put in the arena
   * @param numFood the number of foods to put in the arena
   * @param numFearful the number of bots that will be scaredybois
   * @param hunger whether or not hunger is enabled
   * @param sensitivity the bots' sensitivity to light
   */
  void Reset(int numLights, int numBots, int numFood,
             int numFearful, bool hunger, int sensitivity);
  ...
  bool IsColliding(
    ArenaEntity * const e, ArenaEntity * const other_e);
  ...
  **/
  ...
  /**
   * @brief Update an entity once
   * @param ent the mobile entity to update
   */
  void MobileEntityUpdate(ArenaMobileEntity* ent);

  /**
   * @brief Update an entity based on the other entities in the arena
   * @param ent the mobile entity to update
   */
  void CheckOtherEnts(ArenaMobileEntity* ent);

  std::vector<class ArenaEntity *> get_entities() const { return entities_; }

  /**
   * @brief getters for arena dimensions
   */
  double get_x_dim() { return x_dim_; }
  double get_y_dim() { return y_dim_; }

  /**
   * @brief getter and setter for game_status_
   */
 ...
 private:
  ...
  Light *lightHolder_{nullptr};

  // Placeholder for lights
  Food *foodHolder_{nullptr};

  // Placeholder for automata
  ArenaMobileEntity *botPlaceholder_{nullptr};
  ...
  // Someone needs to be keeping track of the foods
  int ifood_{3};

  bool hunger_active_{true};
};
