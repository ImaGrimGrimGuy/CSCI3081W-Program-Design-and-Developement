 ...
 * Member Functions
 ******************************************************************************/
void ScaredyBoi::TimestepUpdate(unsigned int dt) {
  if (timer_ != 0) {
  ...
  left_light_->set_position(
    get_pose().x + get_radius()*cos(get_pose().theta * pi/180 -pi/2.0),
    get_pose().y + get_radius()*sin(get_pose().theta * pi/180 -pi/2.0));
  ...
  if (((left_food_->get_reading() >= 990)
        || (right_food_->get_reading()) >= 990)) {
    hunger_ = 3600;
  }
...
void ScaredyBoi::HandleCollision(EntityType object_type, ArenaEntity * object) {
  switch (object_type) {
    case(kFood): break;
    case(kLight): break;
    case(kEntity):
