...
void MotionBehaviorDifferential::UpdatePose(double dt, WheelVelocity vel,
  ...
  if (std::fabs(temp_vel_.left - temp_vel_.right) > 0) { /* general case */
    ...
    x_prime = (pose.x - icc.x + offset * rotation) * std::cos(omega() * dt) +
              (pose.y - icc.y + offset * rotation) * -std::sin(omega() * dt)
              + icc.x;
    y_prime = (pose.x - icc.x + offset * rotation) * std::sin(omega() * dt) +
              (pose.y - icc.y + offset * rotation) * std::cos(omega() * dt)
              + icc.y;
